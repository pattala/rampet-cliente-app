// /modules/notifications.js — FCM + VAPID + Opt-In + Geo + Domicilio (lógica pactada, depurada)
'use strict';
...
function hideNotifCards(){ show($('notif-prompt-card'), false); show($('notif-card'), false); }
function hardSyncUI(){ try{refreshNotifUIFromPermission();}catch{} try{syncProfileConsentUI();}catch{} try{syncProfileGeoUI();}catch{} }
...
// GEO: cool-down del banner principal
const LS_GEO_SUPPRESS_UNTIL = 'geoSuppressUntil';
const GEO_COOLDOWN_DAYS     = (window.__RAMPET__?.GEO_COOLDOWN_DAYS ?? 60);

// GEO: “defer” por sesión del banner principal
const GEO_SS_DEFER_KEY      = 'geoBannerDeferred';

// DOMICILIO: “Luego” (sesión) y “No quiero” (persistente)
const LS_ADDR_DEFERRED      = 'addressBannerDeferred';  // sessionStorage
const LS_ADDR_DISMISSED     = 'addressBannerDismissed'; // localStorage
...
function isGeoSuppressedNow(){ try{ return (+localStorage.getItem(LS_GEO_SUPPRESS_UNTIL)||0) > _nowMs(); }catch{ return false; } }
...
let __notifReqInFlight=false, __tokenReqLock=null, __hardResetAttempted=false;
let __tailRetryScheduled=false, __tokenProvisionPending=false;
...
function bootstrapFirstSessionUX(){
  try{
    ...
    // NOTIFS → primera vez sin estado local: card marketing
...
function ensureNotifOffBanner(){
    ...
    <div style="display:flex;align-items:center;gap:10px;justify-content:space-between;flex-wrap:wrap;">
      <div style="display:flex;gap:10px;align-items:center;">
        ...
        <div>
          ...
          Activalas desde <em>Mi Perfil</em> cuando quieras.
        </div>
      </div>
  ...
  $('#notif-off-go-profile')?.addEventListener('click', ()=>{ try{ window.UI?.openProfileModal?.(); }catch{} hardSyncUI(); });
  return el;
}
...
async function ensureMessagingCompatLoaded(){
  ...
  await new Promise((ok, err)=>{ const s=document.createElement('script'); s.src='https://www.gstatic.com/firebasejs/9.6.0/firebase-messaging-compat.js'; s.onload=ok; s.onerror=err; document.head.appendChild(s); });
}
...
async function obtenerYGuardarTokenOneShot(){
  ...
  try{
    ...
    if (!tok) return null;
    await guardarTokenEnMiDoc(tok); try{ refreshNotifUIFromPermission?.(); }catch{}
    return tok;
...
async function obtenerYGuardarToken(){
  ...
  try{
    ...
    if(!reg || !reg.active){
      ...
      try{ const once=()=>{ navigator.serviceWorker.removeEventListener('controllerchange', once); setTimeout(()=>{ obtenerYGuardarToken().catch(()=>{}); },300); }; navigator.serviceWorker.addEventListener('controllerchange', once, { once:true }); }catch{}
...
function refreshNotifUIFromPermission(){
  ...
  // Bloqueado/denegado → OFF total
  if (lsState==='blocked' || perm==='denied'){ if(switchEl) switchEl.checked=false; show(warnBlocked,true); showNotifOffBanner(true); return; }

  // Deferido → mostrar switch OFF (sin marketing)
  if (lsState==='deferred'){ if(switchEl) switchEl.checked=false; if(!pending) show(cardSwitch,true); showNotifOffBanner(false); return; }

  // Concedido → ocultar slider inmediatamente (sin esperar token)
  if (perm==='granted'){
    hideNotifCards();
    const hasToken = isNotifEnabledLocally();
    if (switchEl) switchEl.checked = !!hasToken; // puede quedar OFF hasta que llegue el token
    showNotifOffBanner(!hasToken);
    return;
  }

  // Primera vez real → marketing
  ...
  try{
    if ('permissions' in navigator && navigator.permissions?.query){
      navigator.permissions.query({ name:'notifications' }).then((permStatus)=>{
        __permWatcher.last=permStatus.state; hardSyncUI();
        if (AUTO_RESUBSCRIBE && permStatus.state==='granted' && hasPriorAppConsent() && !isNotifEnabledLocally() && (localStorage.getItem(LS_NOTIF_STATE)!=='blocked')) obtenerYGuardarTokenOneShot().catch(()=>{});
        permStatus.onchange=()=>{ __permWatcher.last=permStatus.state; hardSyncUI();
          if (AUTO_RESUBSCRIBE && permStatus.state==='granted' && hasPriorAppConsent() && !isNotifEnabledLocally() && (localStorage.getItem(LS_NOTIF_STATE)!=='blocked')) obtenerYGuardarTokenOneShot().catch(()=>{});
        };
...
function startPollingWatcher(){
  ...
  __permWatcher.timer=setInterval(()=>{
    ...
    __permWatcher.last=cur; hardSyncUI();
    if (AUTO_RESUBSCRIBE && cur==='granted' && hasPriorAppConsent() && !isNotifEnabledLocally() && (localStorage.getItem(LS_NOTIF_STATE)!=='blocked')) obtenerYGuardarTokenOneShot().catch(()=>{});
  }, 1200);
}
...
export async function handlePermissionRequest(){
  ...
  const sw=$('notif-switch'); disable(sw,true);
  try{
    const current=Notification.permission;
    const ls=(()=>{ try{ return localStorage.getItem(LS_NOTIF_STATE)||null; }catch{ return null; } })();

    if (current==='granted'){
      if (ls==='blocked'){ showNotifOffBanner(true); hardSyncUI(); return; }
      // Ocultar UI al instante y setear accepted
      try{ localStorage.setItem(LS_NOTIF_STATE,'accepted'); }catch{}
      hideNotifCards(); showNotifOffBanner(false); hardSyncUI();
      await obtenerYGuardarToken(); // corre detrás; UI ya cerrada
      return;
    }

    if (current==='denied'){
      try{ localStorage.setItem(LS_NOTIF_STATE,'blocked'); }catch{}
      emit('rampet:consent:notif-opt-out',{source:'browser-denied'}); showNotifOffBanner(true); hardSyncUI(); return;
    }
    ...
    if (status==='granted'){
      try{ localStorage.setItem(LS_NOTIF_STATE,'accepted'); }catch{}
      hideNotifCards(); showNotifOffBanner(false); hardSyncUI();
      // no bloqueo la UI esperando token
      obtenerYGuardarToken().catch(()=>{});
    } else if (status==='denied'){
      try{ localStorage.setItem(LS_NOTIF_STATE,'blocked'); }catch{}
      emit('rampet:consent:notif-opt-out',{source:'prompt'}); showNotifOffBanner(true); hardSyncUI();
    } else {
      try{ localStorage.setItem(LS_NOTIF_STATE,'deferred'); }catch{}
      emit('rampet:consent:notif-dismissed',{}); show($('notif-card'), true); hardSyncUI();
    }
  } catch(e){
    console.warn('[notifications] handlePermissionRequest error:', e?.message||e);
    hardSyncUI();
  } finally { __notifReqInFlight=false; disable(sw,false); }
}
export function handlePermissionBlockClick(){
  try{ localStorage.setItem(LS_NOTIF_STATE,'blocked'); }catch{}
  hideNotifCards(); const sw=$('notif-switch'); if (sw) sw.checked=false;
  setClienteConfigPatch({ notifEnabled:false, notifUpdatedAt:new Date().toISOString() }).catch(()=>{});
  emit('rampet:consent:notif-opt-out',{source:'ui-block'}); toast('Podés volver a activarlas desde tu Perfil.','info');
  showNotifOffBanner(true); hardSyncUI();
}
export function dismissPermissionRequest(){
  try{ localStorage.setItem(LS_NOTIF_STATE,'deferred'); }catch{}
  show($('notif-prompt-card'), false); emit('rampet:consent:notif-dismissed',{});
  const sw=$('notif-switch'); if (sw) sw.checked=false; show($('notif-card'), true); hardSyncUI();
}
export async function handlePermissionSwitch(e){
  const checked=!!e?.target?.checked;
  if (!('Notification' in window)) { hardSyncUI(); return; }
  const before=Notification.permission; const sw=$('notif-switch'); disable(sw,true);
  try{
    if (checked){
      if (before==='granted'){
        try{ localStorage.setItem(LS_NOTIF_STATE,'accepted'); }catch{}
        ...
        if (status==='granted'){
          ...
          try{ localStorage.setItem(LS_NOTIF_STATE,'blocked'); }catch{}
          toast('Notificaciones bloqueadas en el navegador.','warning'); if (sw) sw.checked=false; showNotifOffBanner(true);
        } else {
          try{ localStorage.setItem(LS_NOTIF_STATE,'deferred'); }catch{}
          if (sw) sw.checked=false; show($('notif-card'), true);
        }
      } else {
        toast('Tenés bloqueadas las notificaciones en el navegador.','warning');
        if (sw) sw.checked=false; showNotifOffBanner(true);
      }
    } else {
      await borrarTokenYOptOut(); showNotifOffBanner(true); toast('Notificaciones desactivadas.','info'); show($('notif-card'), true);
    }
  } finally { hardSyncUI(); disable(sw,false); }
}
...
export async function syncProfileConsentUI(){
  ...
  if (perm!=='granted' || ls==='blocked' || ls==='deferred'){ cb.checked=false; cb.dataset.perm=perm; return; }

  const localOn=isNotifEnabledLocally(); let serverOn=null; try{ serverOn=await fetchServerNotifEnabled(); }catch{}
  cb.checked=!!(localOn || serverOn); cb.dataset.perm=perm;
}
    ...
    if (('Notification' in window) && Notification.permission==='granted'){
      ...
      try{
        ...
        if (status==='granted'){
          ...
          try{ localStorage.setItem(LS_NOTIF_STATE,'blocked'); }catch{}
          toast('Notificaciones bloqueadas en el navegador.','warning'); $('prof-consent-notif') && ( $('prof-consent-notif').checked=false ); showNotifOffBanner(true);
        }else{
          try{ localStorage.setItem(LS_NOTIF_STATE,'deferred'); }catch{}
          $('prof-consent-notif') && ( $('prof-consent-notif').checked=false ); show($('notif-card'), true);
        }
...
async function detectGeoPermission(){
  try{ if (navigator.permissions?.query){ const st=await navigator.permissions.query({ name:'geolocation' }); return st.state; } }catch{}
  return 'unknown';
}
...
async function shouldHideGeoBanner(){
  ...
  try{ if (localStorage.getItem(LS_ADDR_DISMISSED)==='1') return true; }catch{}
  return await hasDomicilioOnServer();
}
function hideGeoBanner(){ const { banner }=geoEls(); if (banner) banner.style.display='none'; }

/* Banner chico recordatorio GEO (se mantiene) */
function ensureGeoOffReminder(){
  let el=$('geo-off-reminder'); if (el) return el;
  el=document.createElement('div');
  el.id='geo-off-reminder'; el.className='card'; el.style.cssText='display:none; margin:12px 0;';
  el.innerHTML=`
    <div style="display:flex;align-items:center;gap:10px;justify-content:space-between;flex-wrap:wrap;">
      <div style="display:flex;gap:10px;align-items:center;">
        <span aria-hidden="true" style="font-size:18px;">📍</span>
...
async function maybeShowGeoOffReminder(){ const perm=await detectGeoPermission(); const addr=await hasDomicilioOnServer(); const blocked=isGeoBlockedLocally(); showGeoOffReminder(blocked && perm!=='granted' && !addr); }

/* GEO Banner (sin “Luego”) */
function setGeoMarketingUI(on){
  const { banner, txt, btnOn, btnOff, btnHelp } = geoEls(); if (!banner) return;
  show(banner,on); if (!on) return;
  ...
  if (!nogo){
    ...
    nogo=document.createElement('button'); nogo.id='geo-nothanks-btn'; nogo.className='link-btn'; nogo.textContent='No gracias'; nogo.style.marginLeft='8px'; actions.appendChild(nogo);
  }
  if (!nogo._wired){
    nogo._wired=true;
    nogo.addEventListener('click', async ()=>{
      try{ localStorage.setItem(LS_GEO_STATE,'blocked'); }catch{}
      setGeoSuppress(GEO_COOLDOWN_DAYS); stopGeoWatch();
      await setClienteConfigPatch({ geoEnabled:false, geoUpdatedAt:new Date().toISOString() }).catch(()=>{});
      hideGeoBanner(); toast('Podés activarlo cuando quieras desde tu Perfil.','info');
      emit('rampet:geo:changed',{enabled:false}); showGeoOffReminder(true);
    });
  }
}
function setGeoRegularUI(state){
  const { banner, txt, btnOn, btnOff, btnHelp }=geoEls(); if (!banner) return; show(banner,true);
  if (state==='granted'){ try{ localStorage.setItem(LS_GEO_STATE,'accepted'); }catch{} if (txt) txt.textContent='Listo: ya podés recibir beneficios cerca tuyo.'; showInline(btnOn,false); showInline(btnOff,false); showInline(btnHelp,false); return; }
  if (state==='denied'){ try{ localStorage.setItem(LS_GEO_STATE,'blocked'); }catch{} if (txt) txt.textContent='Para activar beneficios cerca tuyo, habilitalo desde la configuración del navegador.'; showInline(btnOn,false); showInline(btnOff,false); showInline(btnHelp,true); return; }
  if (txt) txt.textContent='Activá para ver beneficios cerca tuyo.'; showInline(btnOn,true); showInline(btnOff,false); showInline(btnHelp,false);
}
...
export async function handleProfileGeoToggle(checked){
  if (checked){ await handleGeoEnable().catch(()=>{}); }
  else{
    try{ localStorage.setItem(LS_GEO_STATE,'blocked'); }catch{}
    setGeoSuppress(GEO_COOLDOWN_DAYS); stopGeoWatch();
    await setClienteConfigPatch({ geoEnabled:false, geoUpdatedAt:new Date().toISOString() }).catch(()=>{});
...
function wireGeoButtonsOnce(){
  const { banner, btnOn, btnOff, btnHelp }=geoEls(); if (!banner || banner._wired) return; banner._wired=true;
...
async function handleGeoEnable(){
  ...
  clearGeoSuppress(); emit('rampet:geo:enabled',{method:'ui'}); startGeoWatch();
...
function handleGeoDisable(){ try{ localStorage.setItem(LS_GEO_STATE,'deferred'); }catch{} emit('rampet:geo:disabled',{method:'ui'}); setClienteConfigPatch({ geoEnabled:false, geoUpdatedAt:new Date().toISOString() }).catch(()=>{}); updateGeoUI(); }
function setGeoOffByUserUI(){ const { banner, txt, btnOn, btnOff, btnHelp }=geoEls(); if (!banner) return; show(banner,true); if (txt) txt.textContent='No vas a recibir beneficios en tu zona. Podés activarlo cuando quieras.'; showInline(btnOn,true); showInline(btnOff,false); showInline(btnHelp, false); }

/* GEO UI global */
async function updateGeoUI(){
  if (isGeoDeferredThisSession()){ hideGeoBanner(); await maybeShowGeoOffReminder(); return; }
  const state=await detectGeoPermission(); const hide=await shouldHideGeoBanner();

  if (isGeoBlockedLocally()){
    stopGeoWatch(); await setClienteConfigPatch({ geoEnabled:false, geoUpdatedAt:new Date().toISOString() });
    hideGeoBanner(); await maybeShowGeoOffReminder(); return;
  }

  if (state==='granted'){
    setGeoMarketingUI(false); startGeoWatch();
    await setClienteConfigPatch({ geoEnabled:true, geoOptInSource:'permission', geoUpdatedAt:new Date().toISOString() });
    if (hide) hideGeoBanner(); else setGeoRegularUI('granted'); showGeoOffReminder(false); return;
  }
  ...
  if (state==='denied'){
    ...
    await maybeShowGeoOffReminder(); return;
  }
...
function startGeoWatch(){ if (!navigator.geolocation || geoWatchId!=null) return; if (isGeoBlockedLocally()) return; if (document.visibilityState!=='visible') return;
  try{ geoWatchId=navigator.geolocation.watchPosition(onGeoPosSuccess,onGeoPosError,{ enableHighAccuracy:false, maximumAge:60000, timeout:10000 }); }catch(e){ console.warn('[geo] start watch error', e?.message||e); } }
...
/* ────────────────────────────────────────────────────────────
   DOMICILIO (banner “📍 Sumá tu domicilio…”) — CON “LUEGO”
   ──────────────────────────────────────────────────────────── */
function ensureAddressBannerButtons(){
  ...
  // Visibilidad inicial
  try{
    if (localStorage.getItem(LS_ADDR_DISMISSED)==='1'){ banner.style.display='none'; return; } // No quiero (persistente)
    if (sessionStorage.getItem(LS_ADDR_DEFERRED)==='1'){ banner.style.display='none'; return; } // Luego (solo esta sesión)
  }catch{}

  const actions=banner.querySelector('.prompt-actions') || banner;

  // LUEGO (preferir botón existente si ya está en el HTML)
  let later = $('address-skip') || $('address-later-btn');
  if (!later){
    later=document.createElement('button');
    later.id='address-later-btn'; later.className='secondary-btn'; later.textContent='Luego'; later.style.marginLeft='8px';
    actions.appendChild(later);
  }
  if (!later._wired){
    later._wired=true;
    later.addEventListener('click', ()=>{
      try{ sessionStorage.setItem(LS_ADDR_DEFERRED,'1'); }catch{}
  ...
  if (!nogo){
    ...
    nogo.id='address-nothanks-btn'; nogo.className='link-btn'; nogo.textContent='No quiero'; nogo.style.marginLeft='8px';
    actions.appendChild(nogo);
  }
  if (!nogo._wired){
    nogo._wired=true;
    nogo.addEventListener('click', ()=>{
      try{ localStorage.setItem(LS_ADDR_DISMISSED,'1'); }catch{}
      banner.style.display='none';
...
export async function initDomicilioForm(){
  ...
  g('address-save')?.addEventListener('click', async ()=>{
    try{
      ...
      try{ localStorage.setItem(LS_ADDR_DISMISSED,'1'); }catch{}
      toast('Domicilio guardado. ¡Gracias!','success'); hideGeoBanner(); updateGeoUI().catch(()=>{}); emit('rampet:geo:changed',{enabled:true});
...
document.addEventListener('visibilitychange', ()=>{ if (document.visibilityState==='visible'){ hardSyncUI(); } });
...
export async function initNotificationsOnce(){
  ...
  bootstrapFirstSessionUX(); startNotifPermissionWatcher();
  ...
  await hookOnMessage(); refreshNotifUIFromPermission(); wirePushButtonsOnce();

  // Perfil
  const profCb=$('prof-consent-notif'); if (profCb && !profCb._wired){ profCb._wired=true; profCb.addEventListener('change',(e)=>handleProfileConsentToggle(!!e.target.checked)); }
  await syncProfileConsentUI();
  const profGeo=$('prof-consent-geo'); if (profGeo && !profGeo._wired){ profGeo._wired=true; profGeo.addEventListener('change',(e)=>handleProfileGeoToggle(!!e.target.checked)); }
