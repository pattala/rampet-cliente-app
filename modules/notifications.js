...
function refreshNotifUIFromPermission() {
  ...
  if (perm==='granted'){
    ...
    if (switchEl) switchEl.checked=false; try { localStorage.setItem(LS_NOTIF_STATE,'blocked'); } catch {}
...
function startPollingWatcher(){
  ...
  __permWatcher.timer = setInterval(()=>{
    const cur=(window.Notification?.permission)||'default'; if (cur===__permWatcher.last) return; __permWatcher.last=cur;
    refreshNotifUIFromPermission(); try { syncProfileConsentUI(); } catch {}
...
export async function handlePermissionRequest() {
  ...
  try{
    ...
    if (current==='denied'){
      ...
      emit('rampet:consent:notif-opt-out',{source:'browser-denied'}); showNotifOffBanner(true); refreshNotifUIFromPermission(); return;
    }
    const status=await Notification.requestPermission(); // default→granted/denied
    if (status==='granted'){ const st=(()=>{ try { return localStorage.getItem(LS_NOTIF_STATE)||null; } catch { return null; } })(); if (st==='blocked'){ showNotifOffBanner(true); } else { await obtenerYGuardarToken(); showNotifOffBanner(false); } }
    else if (status==='denied'){ try { localStorage.setItem(LS_NOTIF_STATE,'blocked'); } catch {} showNotifOffBanner(true); }
    else { try { localStorage.setItem(LS_NOTIF_STATE,'deferred'); } catch {} emit('rampet:consent:notif-dismissed',{}); }
...
export function handlePermissionBlockClick() {
  try { localStorage.setItem(LS_NOTIF_STATE,'blocked'); } catch {}
  show($('notif-prompt-card'), false); const sw=$('notif-switch'); if (sw) sw.checked=false;
  setClienteConfigPatch({ notifEnabled:false, notifUpdatedAt:new Date().toISOString() }).catch(()=>{});
  emit('rampet:consent:notif-opt-out',{source:'ui-block'}); toast('Podés volver a activarlas desde tu Perfil cuando quieras.','info');
...
async function hookOnMessage() {
  try { await ensureMessagingCompatLoaded(); const messaging=firebase.messaging(); messaging.onMessage(async(payload)=>{ const d=payload?.data||{}; try {
      const reg = await navigator.serviceWorker.getRegistration(SW_PATH) || await navigator.serviceWorker.getRegistration();
      if (reg?.showNotification){ await reg.showNotification(d.title||'RAMPET',{ body:d.body||'', icon:d.icon||'/images/mi_logo_192.png', tag:d.tag||d.id||'rampet-fg', data:{ url:d.url||d.click_action||'/?inbox=1' } }); }
    } catch(e){ console.warn('[onMessage] error', e?.message||e); } }); } catch(e){ console.warn('[notifications] hookOnMessage error:', e?.message||e); }
}
...
export async function handleProfileConsentToggle(checked) {
  if (checked){
    if (('Notification' in window) && Notification.permission==='granted'){ try { await obtenerYGuardarToken(); showNotifOffBanner(false); } catch {} }
      ...
      try { const status=await Notification.requestPermission();
        if (status==='granted'){ try { await obtenerYGuardarToken(); showNotifOffBanner(false); } catch {} }
  ...
  } else { await borrarTokenYOptOut(); showNotifOffBanner(true); }
  refreshNotifUIFromPermission();
}
...
// ¿Ocultar banner grande GEO?
async function shouldHideGeoBanner() {
  if (isGeoSuppressedNow()) return true; // cool-down del banner grande
  if (isGeoBlockedLocally()) return false; // dejamos el recordatorio (no ocultar)
...
/* ── Banner chico GEO (recordatorio tras “No gracias”) ── */
...
async function maybeShowGeoOffReminder(){
  ...
  // Mostrar recordatorio SOLO si: usuario opt-out, no hay permiso concedido y no hay domicilio cargado
...
function setGeoMarketingUI(on) {
  ...
  const oldLater = $('#geo-later-btn'); if (oldLater) oldLater.remove();

  // “No gracias” persistente (si falta en el HTML, lo creo)
  ...
  if (!nogo._wired) {
    ...
    nogo.addEventListener('click', async () => {
      ...
      setGeoSuppress(GEO_COOLDOWN_DAYS); // suprime el banner grande
      stopGeoWatch();
...
/* ────────────────────────────────────────────────────────────
   GEO — Perfil (switch) + estado servidor
   ──────────────────────────────────────────────────────────── */
...
function setGeoOffByUserUI(){
  const { banner, txt, btnOn, btnOff, btnHelp } = geoEls();
  if (!banner) return; show(banner,true);
...
async function updateGeoUI() {
  ...
  if (state === 'granted') {
    ...
    hide ? hideGeoBanner() : setGeoRegularUI('granted');
    showGeoOffReminder(false); // permiso ON → sin recordatorio
    return;
  }
...
function buildAddressLine(c) {
  const parts=[]; if (c.calle) parts.push(c.calle + (c.numero ? ' ' + c.numero : ''));
...
/* Domicilio: asegurar botones y wiring */
...
document.addEventListener('visibilitychange', ()=>{
  if (document.visibilityState==='visible'){ syncProfileConsentUI(); syncProfileGeoUI(); maybeShowGeoOffReminder(); }
});
...
export async function initNotificationsOnce() {
  ...
  // asegurar estado del recordatorio GEO
