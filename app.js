...
function cleanupListener() { if (unsubscribeCliente) unsubscribeCliente(); clienteData = null; clienteRef = null; premiosData = []; }
async function listenToClientData(user) { if (unsubscribeCliente) unsubscribeCliente(); const clienteQuery = db.collection('clientes').where("authUID", "==", user.uid).limit(1); unsubscribeCliente = clienteQuery.onSnapshot(async (snapshot) => { if (snapshot.empty) { showToast("Error: Tu cuenta no está vinculada a ninguna ficha de cliente.", "error"); logout(); return; } const doc = snapshot.docs[0]; clienteData = doc.data(); clienteRef = doc.ref; try { if (premiosData.length === 0) { const premiosSnapshot = await db.collection('premios').orderBy('puntos', 'asc').get(); premiosData = premiosSnapshot.docs.map(p => p.data()); } const hoy = new Date().toISOString().split('T')[0]; const campanasSnapshot = await db.collection('campanas').where('estaActiva', '==', true).where('fechaInicio', '<=', hoy).get(); const campanasVisibles = campanasSnapshot.docs.map(doc => doc.data()).filter(campana => hoy <= campana.fechaFin); renderMainScreen(clienteData, premiosData, campanasVisibles); } catch (e) { console.error("Error cargando datos adicionales:", e); renderMainScreen(clienteData, premiosData, []); } gestionarPermisoNotificaciones(clienteData); }, (error) => { console.error("Error en listener de cliente:", error); logout(); }); }
async function acceptTerms() { if (!clienteRef) return; const boton = document.getElementById('accept-terms-btn-modal'); boton.disabled = true; try { await clienteRef.update({ terminosAceptados: true }); showToast("¡Gracias por aceptar los términos!", "success"); closeTermsModal(); } catch (error) { showToast("No se pudo actualizar. Inténtalo de nuevo.", "error"); } finally { boton.disabled = false; } }
